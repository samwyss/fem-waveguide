...
def main() -> None:
    ...
    """
    plt.plot(k0_th_a, np.sqrt((2*np.pi*frequencies_th)**2 * mu * epsilon - (1.841/a)**2) / k0_th, "b")
    ...
    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TE[1])) / k0_s, "b1", label=r"$TE_{11}$")
    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TE[3])) / k0_s, "r3", label=r"$TE_{21}$")
    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TE[5])) / k0_s, "g2", label=r"$TE_{01}$")

    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TM[0])) / k0_s, "c4", label=r"$TM_{01}$")
    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TM[1])) / k0_s, "m+", label=r"$TM_{11}$")
    plt.plot(k0_s_a, np.sqrt((2*np.pi*frequencies_s)**2 * mu * epsilon - np.sqrt(first_eig_vals_TM[3])) / k0_s, "yx", label=r"$TM_{21}$")
